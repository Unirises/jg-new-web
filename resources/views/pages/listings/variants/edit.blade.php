...
@slot('title') Update Item Variant @endslot

@endcomponent
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <form action="{{ route('category.item.variants.update', [$category, $item, $variant]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    @if($errors->any())
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="min" name="min" value="{{ old('min') ?? $variant->min ?? '' }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="max" class="form-label">Maximum # of Selection</label>
                        <input type="text" class="form-control" id="max" name="max" value="{{ old('max') ?? $variant->max ?? '' }}" required>
                    </div>
                </div>
